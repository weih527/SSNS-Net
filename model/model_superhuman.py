...
class UNet_PNI_Noskip2(nn.Module):  # deployed PNI model
    ...
    def __init__(self, in_planes=1, 
        ...
        self.cat0 = conv3dBlock([0], [filters2[4]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
        # self.conv4 = resBlock_pni(filters2[4], filters2[4], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        self.conv4 = conv3dBlock([filters2[4]], [filters2[4]], [(3,3,3)], [1], [(1,1,1)], [True], [pad_mode], [bn_mode], [relu_mode], init_mode, bn_momentum)

        self.up1 = upsampleBlock(filters2[4], filters2[3], (1,2,2), upsample_mode, init_mode=init_mode)
        self.cat1 = conv3dBlock([0], [filters2[3]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
        # self.conv5 = resBlock_pni(filters2[3], filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        self.conv5 = conv3dBlock([filters2[3]], [filters2[3]], [(3,3,3)], [1], [(1,1,1)], [True], [pad_mode], [bn_mode], [relu_mode], init_mode, bn_momentum)

        self.up2 = upsampleBlock(filters2[3], filters2[2], (1,2,2), upsample_mode, init_mode=init_mode)
        self.cat2 = conv3dBlock([0], [filters2[2]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
        # self.conv6 = resBlock_pni(filters2[2], filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        self.conv6 = conv3dBlock([filters2[2]], [filters2[2]], [(3,3,3)], [1], [(1,1,1)], [True], [pad_mode], [bn_mode], [relu_mode], init_mode, bn_momentum)

        self.up3 = upsampleBlock(filters2[2], filters2[1], (1,2,2), upsample_mode, init_mode=init_mode)
        self.cat3 = conv3dBlock([0], [filters2[1]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
        # self.conv7 = resBlock_pni(filters2[1], filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        self.conv7 = conv3dBlock([filters2[1]], [filters2[1]], [(3,3,3)], [1], [(1,1,1)], [True], [pad_mode], [bn_mode], [relu_mode], init_mode, bn_momentum)
...
class UNet_PNI_FT1(nn.Module):  # deployed PNI model
    ...
    def __init__(self, pretrained_model, 
        ...
        if self.merge_mode == 'add' or self.merge_mode == 'add2' or self.merge_mode == 'add3':
            self.cat0_ft = conv3dBlock([0], [filters2[4]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            ...
            self.cat0_ft = conv3dBlock([0], [filters2[4]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv4_ft = resBlock_pni(filters2[4]*2, filters2[4], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat3':
            self.cat0_ft = conv3dBlock([0], [filters2[4]*3], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv4_ft = resBlock_pni(filters2[4]*3, filters2[4], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
        ...
        if self.merge_mode == 'add' or self.merge_mode == 'add2' or self.merge_mode == 'add3':
            self.cat1_ft = conv3dBlock([0], [filters2[3]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv5_ft = resBlock_pni(filters2[3], filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat' or self.merge_mode == 'cat2':
            self.cat1_ft = conv3dBlock([0], [filters2[3]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv5_ft = resBlock_pni(filters2[3]*2, filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat3':
            self.cat1_ft = conv3dBlock([0], [filters2[3]*3], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv5_ft = resBlock_pni(filters2[3]*3, filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            raise AttributeError('No this merge mode!')

        self.up2_ft = upsampleBlock(filters2[3], filters2[2], (1,2,2), upsample_mode, init_mode=init_mode)
        if self.merge_mode == 'add' or self.merge_mode == 'add2' or self.merge_mode == 'add3':
            self.cat2_ft = conv3dBlock([0], [filters2[2]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv6_ft = resBlock_pni(filters2[2], filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat' or self.merge_mode == 'cat2':
            self.cat2_ft = conv3dBlock([0], [filters2[2]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv6_ft = resBlock_pni(filters2[2]*2, filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat3':
            self.cat2_ft = conv3dBlock([0], [filters2[2]*3], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv6_ft = resBlock_pni(filters2[2]*3, filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            raise AttributeError('No this merge mode!')

        self.up3_ft = upsampleBlock(filters2[2], filters2[1], (1,2,2), upsample_mode, init_mode=init_mode)
        if self.merge_mode == 'add' or self.merge_mode == 'add2' or self.merge_mode == 'add3':
            self.cat3_ft = conv3dBlock([0], [filters2[1]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv7_ft = resBlock_pni(filters2[1], filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat' or self.merge_mode == 'cat2':
            self.cat3_ft = conv3dBlock([0], [filters2[1]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv7_ft = resBlock_pni(filters2[1]*2, filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        elif self.merge_mode == 'cat3':
            self.cat3_ft = conv3dBlock([0], [filters2[1]*3], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv7_ft = resBlock_pni(filters2[1]*3, filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            raise AttributeError('No this merge mode!')
