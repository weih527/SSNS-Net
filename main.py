...
from dataloader.data_provider_unlabel import Provider as Provider_unlabel
...
from utils.consistency_aug import convert_consistency_scale, convert_consistency_flip, convert_consistency_rot
...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        inputs_unlabel_aug, inputs_unlabel_gt, det_sizes, rules, rotnums = train_provider_unlabel.next()
        ...
        with torch.no_grad():
            pred_unlabel_gt = model(inputs_unlabel_gt)

        # convert rotation
        if cfg.DATA.if_rotation_aug_unlabel:
            pred_unlabel_gt = convert_consistency_rot(pred_unlabel_gt, rotnums)
        # convert flip
        ...
        if cfg.TRAIN.if_valid:
            if iters % cfg.TRAIN.save_freq == 0 and iters >= cfg.TRAIN.min_valid_iters:
                ...
                #     cmd_val = ['python2','evaluate_mala_models.py', '-in',cfg.record_path, '-gt',cfg.DATA.data_folder, 
